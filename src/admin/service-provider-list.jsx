...
const VendorList = ({ className, sideOptionSelected, ...rest }) => {
  ...
  useEffect(() => {
    ...
    // axios
    //   .get(
    //     configs.serverUrl + "admin/getAllServiceProviders/",
    //     //  + pagedata + "/20"
    //     {
    //       headers: {
    //         Authorization: token,
  ...
  const editThisFeed = (providerId, providerDetails) => {
    setCurrentUser(providerId);
  ...
  const handleDelete = (value) => {
    ...
    axios
      .post(
        `${Config.serverUrl}admin/deleteServiceProvider`,
        {
          userId: value,
        },
        {
  ...
  return (
    <Page className={classes.root} title={"Service Provider List"}>
      {!userForm && (
      ...
      {userForm && !currentUserId ? <AddServiceProvider /> : ""}
